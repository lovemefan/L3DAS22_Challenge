...
class DepthWiseConv2d(nn.Module):
    def __init__(self, chan_in, chan_out, kernel_size, padding):
        ...
        self.conv = nn.Conv2d(chan_in, chan_out, kernel_size, groups = chan_in)
...
class PreNorm(nn.Module):
    ...
    def forward(self, x, **kwargs):
        # h, w = x.shape[-2:]
        # x = self.to_4d(self.body(self.to_3d(x)), h, w)
        if self.fn is None:
            return x
        else:
...
class ConformerConvModule(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(dim, inner_dim * 2, 1)
        self.activate1 = GLU(dim=1)
        self.dconv = DepthWiseConv2d(inner_dim, inner_dim, kernel_size=1, padding=(0,0))
        self.bacth_norm = nn.BatchNorm2d(inner_dim) if not causal else nn.Identity()
        self.activate2 = Swish()
        self.conv2 = nn.Conv2d(inner_dim, dim, 1)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x):
        ...
        x = self.activate1(self.conv1(x))
        x = self.dconv(x)
        x = self.activate2(self.bacth_norm(x))
        x = self.conv2(x)
